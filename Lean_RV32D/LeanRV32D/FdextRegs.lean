...
def rF_or_X_D (i : fregidx) : SailM (BitVec 64) := do
  ...
  then (rF_D i)
    ...
    (do
      ...
      assert ((BitVec.access (regidx_bits ridx) 0) == 0#1) "extensions/FD/fdext_regs.sail:297.42-297.43"
      if ((ridx == zreg) : Bool)
...
def wF_or_X_D (i : fregidx) (data : (BitVec 64)) : SailM Unit := do
  ...
  then (wF_D i data)
    ...
    (do
      let ridx := (fregidx_to_regidx i)
      assert ((BitVec.access (regidx_bits ridx) 0) == 0#1) "extensions/FD/fdext_regs.sail:330.43-330.44"
