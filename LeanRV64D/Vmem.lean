...
/-- Type quantifiers: k_ex662621_ : Bool, level : Nat, k_ex662619_ : Bool, k_ex662618_ : Bool, sv_width
...
def pt_walk (sv_width : Nat) (vpn : (BitVec (sv_width - 12))) (ac : (MemoryAccessType Unit)) (priv : Privilege) (mxr : Bool) (do_sum : Bool) (pt_base : (BitVec (if ( sv_width
  ...
  assert ((sv_width == 32) || (xlen == 64)) "sys/vmem.sail:105.36-105.37"
...
/-- Type quantifiers: tlb_index : Nat, k_ex662674_ : Bool, k_ex662673_ : Bool, sv_width : Nat, is_sv_mode(sv_width), 0
        ...
        (do
          ...
          then (pure (Err ((PTW_PTE_Needs_Update ()), ext_ptw)))
            ...
            (do
              ...
              | .Ok _ => (pure ())
              | .Err _ => (internal_error "sys/vmem.sail" 273 "invalid physical address in TLB")
              (pure (Ok ((tlb_get_ppn sv_width ent vpn), ext_ptw))))))

/-- Type quantifiers: k_ex662695_ : Bool, k_ex662694_ : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
...
/-- Type quantifiers: k_ex662731_ : Bool, k_ex662730_ : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
...
def get_satp (sv_width : Nat) : SailM (BitVec (if ( sv_width = 32  : Bool) then 32 else 64)) := do
  assert ((sv_width == 32) || (xlen == 64)) "sys/vmem.sail:370.30-370.31"
  if ((sv_width == 32) : Bool)
...
def translateAddr (vAddr : virtaddr) (ac : (MemoryAccessType Unit)) : SailM (Result (physaddr × Unit) (ExceptionType × Unit)) := do
  ...
  then (pure (Ok ((Physaddr (zero_extend (m := 64) (bits_of_virtaddr vAddr))), init_ext_ptw)))
    ...
    (do
      ...
      assert ((sv_width == 32) || (xlen == 64)) "sys/vmem.sail:396.36-396.37"
