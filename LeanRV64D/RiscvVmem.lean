...
/-- Type quantifiers: k_ex376866# : Bool, level : Nat, k_ex376864# : Bool, k_ex376863# : Bool, sv_width
  ...
  then (pure Bare)
    ...
    (do
      ...
      let mbits ← (( do
        ...
        | RV128 => (internal_error "./riscv_vmem.sail" 196 "RV128 not supported") ) : SailM
        satp_mode )
      match (satpMode_of_bits arch mbits) with
      | .some m => (pure m)
      | none => (internal_error "./riscv_vmem.sail" 201 "invalid translation mode in satp"))

/-- Type quantifiers: tlb_index : Nat, k_ex376911# : Bool, k_ex376910# : Bool, sv_width : Nat, is_sv_mode(sv_width), 0
...
/-- Type quantifiers: k_ex376932# : Bool, k_ex376931# : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
...
/-- Type quantifiers: k_ex376968# : Bool, k_ex376967# : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
