...
/-- Type quantifiers: k_ex381085# : Bool, k_m : Nat, k_m ≥ 0, k_m ∈ {16, 32, 64} -/
  ...
  let (normalized_exp, normalized_sig) ← do
    ...
    then
      (do
        ...
        assert (nr_leadingzeros ≥b 0) "riscv_insts_vext_fp_utils.sail:478.35-478.36"
  ...
  let idx : Nat :=
    ...
    | _ =>
      (BitVec.toNat
        ((BitVec.join1 [(BitVec.access normalized_exp 0)]) ++ (Sail.BitVec.extractLsb normalized_sig
            51 46)))
  assert ((idx ≥b 0) && (idx <b 128)) "riscv_insts_vext_fp_utils.sail:489.29-489.30"
...
/-- Type quantifiers: k_ex381321# : Bool, k_m : Nat, k_m ≥ 0, k_m ∈ {16, 32, 64} -/
def recip7 (v : (BitVec k_m)) (rm_3b : (BitVec 3)) (sub : Bool) : SailM (Bool × (BitVec 64)) := do
  ...
  let nr_leadingzeros := (BitVec.countLeadingZeros (Sail.BitVec.extractLsb sig (s -i 1) 0))
  assert (nr_leadingzeros ≥b 0) "riscv_insts_vext_fp_utils.sail:569.29-569.30"
  ...
  assert ((idx ≥b 0) && (idx <b 128)) "riscv_insts_vext_fp_utils.sail:582.29-582.30"
  let mid_exp :=
