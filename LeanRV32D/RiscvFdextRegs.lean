...
def rF_or_X_D (i : fregidx) : SailM (BitVec 64) := do
  ...
  then (rF_D i)
    ...
    (do
      ...
      assert ((BitVec.access (regidx_bits ridx) 0) == 0#1) "riscv_fdext_regs.sail:287.42-287.43"
      if ((ridx == zreg) : Bool)
...
def wF_or_X_D (i : fregidx) (data : (BitVec 64)) : SailM Unit := do
  ...
  then (wF_D i data)
    ...
    (do
      let ridx := (fregidx_to_regidx i)
      assert ((BitVec.access (regidx_bits ridx) 0) == 0#1) "riscv_fdext_regs.sail:320.43-320.44"
